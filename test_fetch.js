...
http
  .get(
    'http://localhost:5050/api/projects/eae678e5-57bc-43b7-9d6a-dbf212cf4802/work-items/backlog',
    (res) => {
        ...
        try {
          ...
          if (data.data && data.data.sprints) {
            data.data.sprints.forEach((s) => {
              console.log(
                `SprintId: ${s.sprintId}, Name: ${s.name}, Status: ${s.status}, Items: ${s.items ? s.items.length : 'NO ITEMS'}`,
              );
            });
          } else {
