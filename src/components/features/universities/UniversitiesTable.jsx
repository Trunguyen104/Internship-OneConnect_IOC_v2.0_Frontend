      ...
      <Table>
        ...
        <TableBody>
          {loading ? (
            ...
            <TableRow key={uni.universityId} className='group h-[72px] border-slate-50 transition-all duration-200 hover:bg-slate-50/80'>
              ...
              <TableCell>
                <div className='flex items-center gap-3'>
                  {uni.logoUrl && (
                    <img src={uni.logoUrl} alt={uni.name} className='h-8 w-8 rounded-lg object-contain bg-slate-50 border border-slate-100 p-1' />
                  )}
                  <span className='text-[15px] font-bold leading-tight text-slate-800'>{uni.name}</span>
                </div>
              </TableCell>
              <TableCell className='hidden lg:table-cell text-[14px] font-medium text-slate-600 max-w-md truncate'>
                {uni.address}
              </TableCell>
