...
const ViolationCard = ({ violation }) => {
  return (
    <Card className="animate-in fade-in slide-in-from-left-4 duration-500 !p-0 border border-slate-100 shadow-lg shadow-slate-200/30 hover:shadow-xl transition-all !rounded-[24px] bg-white overflow-hidden group h-fit !min-h-0">
      <div className="flex flex-col md:flex-row md:items-stretch h-fit !min-h-0">
        ...
        <div className="flex-1 p-6 flex flex-col gap-5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col min-w-0">
                <h4 className="text-base font-black text-slate-800 tracking-tight truncate leading-tight">
            ...
            <div className="px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-100 flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase tracking-widest hidden sm:flex">
              <HistoryOutlined />
              {STUDENT_ACTIVITY_UI.VIOLATIONS.READ_ONLY_ARCHIVE}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-[9px] font-black text-slate-400 grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100 transition-all">
...
export default function ViolationsTab({ violations, loading }) {
  ...
  if (!violations || violations.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-8 bg-white/80 backdrop-blur-sm rounded-[32px] animate-in zoom-in-95 duration-1000 shadow-xl shadow-slate-200/40 border border-white mt-[-20px]">
        <div className="size-16 rounded-[24px] bg-slate-50 flex items-center justify-center text-slate-200 text-3xl mb-6 shadow-inner border border-slate-100/50">
          <HistoryOutlined />
        </div>
        <h3 className="text-lg font-black text-slate-400 tracking-tight uppercase px-12 text-center leading-tight">
          {STUDENT_ACTIVITY_UI.VIOLATIONS.NO_VIOLATIONS_MATCH}
        </h3>
        <span className="text-[10px] font-black text-slate-300 mt-4 uppercase tracking-[0.2em] italic bg-slate-50/50 px-6 py-2 rounded-full border border-slate-50">
  ...
  return (
    <div className="flex flex-col gap-6 animate-in -mt-4 fade-in slide-in-from-bottom-4 duration-700">
      {violations.map((v, i) => (
