          ...
          {loading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <TableRow key={i} className='h-[72px] border-slate-50'>
                <TableCell className='pl-8 w-32'><Skeleton className='h-4 w-20' /></TableCell>
                <TableCell>
            ...
            enterprises.map((ent) => (
            <TableRow key={ent.enterpriseId || ent.id} className='group h-[72px] border-slate-50 transition-all duration-200 hover:bg-slate-50/80'>
              ...
              <TableCell>
                <div className='flex items-center gap-3'>
                  <div className='h-10 w-10 rounded-lg overflow-hidden bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0'>
                    {ent.logoUrl ? (
                      <img src={ent.logoUrl} alt={ent.name} className='h-full w-full object-contain p-1' />
                    ) : (
                      <div className='text-primary font-bold text-sm'>{ent.name?.[0]}</div>
                    )}
                  </div>
                  <div className='flex flex-col'>
                    <span className='text-[15px] font-bold leading-tight text-slate-800'>{ent.name}</span>
                    <span className='text-[12px] font-medium text-slate-400 truncate max-w-[200px]'>{ent.website}</span>
                  </div>
                </div>
              </TableCell>
              <TableCell className='hidden lg:table-cell'>
                <span className='inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wider border border-primary/10 bg-primary/5 text-primary'>
