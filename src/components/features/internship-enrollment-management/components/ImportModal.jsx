  ...
  const columns = [
    {
      ...
      dataIndex: 'fullName',
      render: (text) => <span className="text-text text-xs font-bold">{text}</span>,
    },
    {
      title: IMPORT.PREVIEW_COLUMNS.STUDENT_ID,
      dataIndex: 'studentCode',
      width: 120,
      render: (text) => <span className="text-muted font-mono text-[10px] font-bold">{text}</span>,
    },
    {
      ...
      render: (_, r) =>
        r.isValid ? (
          ...
          <Tooltip title={r.errors?.join(', ')}>
            <ExclamationCircleOutlined className="text-danger text-base" />
          </Tooltip>
        ),
    },
  ];
