...
export default function CompleteSprintModal({ open, sprint, sprints, onClose, onSubmit }) {
    ...
    const futureSprints = useMemo(() => {
        return (sprints || []).filter(s =>
            ...
            (!s.status || s.status.toUpperCase() === 'PLANNED')
        );
    }, [sprints, sprint]);
    ...
    return (
        <div className='fixed inset-0 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm bg-black/40'>
            <div className='relative w-full max-w-[500px] rounded-3xl bg-white shadow-2xl p-8 flex flex-col'>
                ...
                <p className='text-sm text-gray-500 mb-6 font-medium'>Có {undoneItems.length} issue chưa xong sẽ được di chuyển:</p>
                ...
                <div className='space-y-4 mb-8'>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-transparent hover:bg-gray-50 transition-colors">
                        <input type="radio" checked={moveOption === 'backlog'} onChange={() => setMoveOption('backlog')} className="w-4 h-4 text-[#A32A2A] focus:ring-[#A32A2A]" />
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-transparent hover:bg-gray-50 transition-colors">
                            <input type="radio" checked={moveOption === 'next'} onChange={() => setMoveOption('next')} className="w-4 h-4 text-[#A32A2A] focus:ring-[#A32A2A]" />
                        ...
                        {moveOption === 'next' && (
                            <div className="ml-9 animate-in slide-in-from-left-2">
                                ...
                                >
                                    {futureSprints.map(s => (
                                        <option key={s.sprintId} value={s.sprintId}>{s.name || s.title}</option>
                                    ))}
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-transparent hover:bg-gray-50 transition-colors">
                            <input type="radio" checked={moveOption === 'new'} onChange={() => setMoveOption('new')} className="w-4 h-4 text-[#A32A2A] focus:ring-[#A32A2A]" />
                ...
                <div className='flex justify-end gap-3 pt-6 border-t border-gray-100'>
                    <button onClick={onClose} className='h-11 px-6 rounded-full font-bold text-gray-500 hover:bg-gray-100'>Hủy</button>
                    <button
