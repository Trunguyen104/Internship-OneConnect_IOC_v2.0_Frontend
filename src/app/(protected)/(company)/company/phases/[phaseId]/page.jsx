...
export default function CompanyPhaseDetailPage() {
  const { PAGE } = INTERN_PHASE_MANAGEMENT;
  const params = useParams();
  const router = useRouter();
  const id = params.phaseId; // Next.js param is 'phaseId' not 'id'
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <CompanyTopNav />
      <main className="flex-1 overflow-auto bg-gray-50 p-4 2xl:p-6 w-full max-w-7xl mx-auto">
        {content}
      </main>
    </div>
  );
